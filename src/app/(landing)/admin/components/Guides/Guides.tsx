...
import { getGuidesAdmin } from "@/lib/adminActions/getAllListings";
...
export default function Guides() {
  ...
  const [listings, setListings] = useState<ProductGuides[]>();
  const [userToDelete, setUserToDelete] = useState<ProductGuides>();
  const { isOpen, onOpen, onClose } = useDisclosure();
  const [success, setSuccess] = useState<boolean>(false);
  const [action, setAction] = useState<string>();

  const handleCity = async (listing: ProductGuides) => {
  ...
  useEffect(() => {
    (async () => {
      const result = (await getGuidesAdmin()) as ProductGuides[];
  ...
  //Table cells
  const renderCell = (listing: ProductGuides, columnKey: string | number) => {
    switch (columnKey) {
      case "Info":
        return (
          ...
          <div className="relative flex items-center gap-2 justify-between">
            <div className="flex flex-row items-center gap-3">
              ...
              <DeleteItem
                ...
                CategoryName="Guides"
              />
              <MoreInfo CategoryName="Guides" listing={listing} />
            </div>
            <Approve
              setSuccess={setSuccess}
              listing={listing}
